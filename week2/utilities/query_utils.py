...
def add_click_priors(query_obj, user_query, priors_gb):
    try:
        ...
        if prior_clicks_for_query is not None and len(prior_clicks_for_query) > 0:
            ...
            counts = prior_clicks_for_query["sku"].value_counts().reset_index()
            tot = counts["sku"].sum()
            n_top = 0
            for row in counts.itertuples():
                boost_amount = round(row.sku / tot, 4)
                if boost_amount:
                    sku = row.index
                    click_prior = f"{click_prior} {sku}^{boost_amount}"
                else:
                    break
            ...
            if click_prior != "":
                click_prior = click_prior.rstrip() # remove the trailing space
                click_prior_query_obj = {"query_string": {"query": click_prior, "fields": ["sku"], "boost": 2500}}
